		...
		if(!!this.offers && 0 < this.offers.length) {
			for(i = 0; i < this.offers.length; i++) {
				...
				if(!!BX('catalog-item-delay-new-'+this.offers[i].ID)) {
					MinselDelayOnclick = BX('catalog-item-delay-new-'+this.offers[i].ID).getAttribute('onclick');
					...
					BX('catalog-item-delay-new-'+this.offers[i].ID).setAttribute('onclick', MinselDelayOnclickNew);
				}
				/*ASK_PRICE*/
				if(!!BX('ask_price_message_new_'+this.offers[i].ID)) {
