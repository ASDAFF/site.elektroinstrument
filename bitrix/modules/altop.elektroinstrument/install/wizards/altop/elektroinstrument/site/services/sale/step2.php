...
if(COption::GetOptionString("altop.elektroinstrument", "wizard_installed", "N", WIZARD_SITE_ID) != "Y") {
	$locationGroupID = 0;
	...
	if(!$dbDelivery->Fetch()) {
		...
		$arFields = Array(
			...
			"LOGOTIP"   => CFile::MakeFileArray($_SERVER['DOCUMENT_ROOT'].WIZARD_SERVICE_RELATIVE_PATH."/sale/delivery/logotip/courier.jpg"),
		);
		...
		$arFields = Array(
			...
			"LOCATIONS" => $arLocation4Delivery,
			"LOGOTIP"   => CFile::MakeFileArray($_SERVER['DOCUMENT_ROOT'].WIZARD_SERVICE_RELATIVE_PATH."/sale/delivery/logotip/pickup.jpg"),
		);
		if($delivery["self"] != "Y")
			$arFields["ACTIVE"] = "N";
		CSaleDelivery::Add($arFields);
...
COption::SetOptionString("altop.elektroinstrument", "wizard_installed", "Y", false, WIZARD_SITE_ID);
?>