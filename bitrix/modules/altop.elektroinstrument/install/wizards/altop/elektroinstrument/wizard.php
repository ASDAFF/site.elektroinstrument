			...
			if(!$db_res->Fetch()) {
				...
				<div class="wizard-input-form-block">
					...
					<div class="wizard-input-form-block-content">
						...
						'. $this->ShowCheckboxField("installPriceBASE[base]", "Y", 
...
class ShopSettings extends CWizardStep {
	function InitStep() {
		...
		$siteStamp =$wizard->GetPath()."/site/templates/elektro/themes/light/images/pechat.gif";
		$siteID = $wizard->GetVar("siteID");
		...
		$wizard->SetDefaultVars(
			Array(
				...
				"siteStamp" => COption::GetOptionString("elektroinstrument", "siteStamp", $siteStamp, $siteID),
			)
		);
	}
