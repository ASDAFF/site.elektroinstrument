					...
					<div class="catalog-item-desc">
						...
						<div class="compare_delay">
							<?if($arParams["DISPLAY_COMPARE"]=="Y"):?>
								<div class="compare">
									<a href="javascript:void(0)" class="catalog-item-compare" id="catalog_add2compare_link_<?=$arElement['ID']?>" onclick="return addToCompare('<?=$arElement["COMPARE_URL"]?>', 'catalog_add2compare_link_<?=$arElement["ID"]?>');" rel="nofollow"><i class="fa fa-bar-chart"></i><i class="fa fa-check"></i><span><?=GetMessage('CATALOG_ELEMENT_ADD_TO_COMPARE')?></span></a>
								</div>
							<?endif;?>
							<?if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):
								if($arElement["OFFERS_MIN_PRICE"]["CAN_BUY"]):
									if($arElement["OFFERS_MIN_PRICE"]["VALUE"] > 0):
										...
										<div class="delay">
											<a href="javascript:void(0)" id="catalog-item-delay-<?=$arElement['OFFERS_MIN_PRICE']['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arElement["OFFERS_MIN_PRICE"]["ID"]?>', '<?=$arElement["OFFERS_MIN_PRICE"]["CATALOG_MEASURE_RATIO"]?>', '<?=$props?>', '', 'catalog-item-delay-<?=$arElement["OFFERS_MIN_PRICE"]["ID"]?>')" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i><span><?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?></span></a>
										</div>
									<?endif;
								endif;
							else:
								if($arElement["CAN_BUY"]):
									foreach($arElement["PRICES"] as $code=>$arPrice):
										if($arPrice["MIN_PRICE"] == "Y"):
											if($arPrice["VALUE"] > 0):?>
												<div class="delay">
													<a href="javascript:void(0)" id="catalog-item-delay-<?=$arElement['ID']?>" class="catalog-item-delay" onclick="return addToDelay('<?=$arElement["ID"]?>', '<?=$arElement["CATALOG_MEASURE_RATIO"]?>', '', '', 'catalog-item-delay-<?=$arElement["ID"]?>')" rel="nofollow"><i class="fa fa-heart-o"></i><i class="fa fa-check"></i><span><?=GetMessage('CATALOG_ELEMENT_ADD_TO_DELAY')?></span></a>
												</div>
											<?endif;
										endif;
									endforeach;
						...
						<?if(isset($arElement["OFFERS"]) && !empty($arElement["OFFERS"])):
							...
							<?elseif($arElement["OFFERS_MIN_PRICE"]["DISCOUNT_VALUE"] < $arElement["OFFERS_MIN_PRICE"]["VALUE"]):?>
								...
								<span class="catalog-item-price-percent">
									<?=GetMessage("CATALOG_ELEMENT_SKIDKA")."<br />".$arElement["OFFERS_MIN_PRICE"]["PRINT_DISCOUNT_DIFF"];?>
								</span>
